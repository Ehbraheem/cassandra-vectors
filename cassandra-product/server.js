...
import { vectorizeText, connectToCassandra, closeCassandraConnection, CASSANDRA_KEYSPACE, CASSANDRA_TABLE } from '../utils/util.js';
...
app.post('/search', async (req, res) => {
  ...
  try {
    ...
    const query = `SELECT reviews_title, brand, similarity_cosine(embedding,  [${queryVector}]) as similarity FROM ${CASSANDRA_KEYSPACE}.${CASSANDRA_TABLE} ORDER BY embedding ANN OF [${queryVector}] LIMIT 3`;
